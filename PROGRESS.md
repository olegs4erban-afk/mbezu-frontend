...
1. **Названия ST-02 и ST-08 перепутаны местами между витриной и Store.** Сличил четыре файла глазами:
   ...
   Клиент выбирает в каталоге одну работу, а на странице товара получает другую под тем же названием. Имена файлов в Store тоже перекрёстные. Чинится: имена в Store-админке + `data.ts`, но **какое имя какой картине принадлежит — должна подтвердить художник**.
2. **`cdn.mbezu.ru/assets/cards/mn-02.webp` → 404.** Из-за этого MN-02 «Вершина» не рисуется в сетке: на `/catalog` **21 карточка**, в `data.ts` — 22 работы, в Store и `sitemap-store.xml` — 22 товара. Работа продаётся, но в каталоге её нет.
...
6. На каждой странице ровно одна картинка **без `alt`**.
...
5. **Крошек нет** ни в каталоге, ни на товаре; вернуться с товара в каталог штатным способом нельзя.
...
3. **Ни одно поле CTA не обязательное** (`required = false` у имени, контакта и текста) — заявка уходит пустой.
