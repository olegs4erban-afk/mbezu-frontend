...
## Sprint 5 — статус
- **Stage A завершён** (чанки исправлены + редеплой CDN + проверка). Tilda пока НЕ тронута. Дальше — Phase 0 (мастер-снимок) на проде.
- Блокер из Фазы -1 (routing `.html` vs чистые алиасы) — **устранён**: routeToPath→чистые, пререндер dir-style, CDN отдаёт чистые URL и навигация кликабельна.
