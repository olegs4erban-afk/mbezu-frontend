...
## Sprint 8 — статус
- **Phase 1 (ГЕЙТ) выполнен → master-таблица на проверке Олегу. Цены НЕ применялись.** Phase 3 (картинки/+40%/чекаут) — только после «ок».
- 2A (золото)/2C (commission) готовы, но требуют redeploy+reswap (как и Phase 3) → эффективнее батчить с Phase 3 (1 редеплой вместо 3). 2B (фото автора) — **заблокировано: `about-author.jpg` отсутствует** в проекте.
