...
- `[fixed] llms.txt` — в отчёте и PROGRESS (строка «Аудит r2») значился как сделанный, но файла в репо не было (только слепки аудита) → cdn.mbezu.ru/llms.txt отдавал 404. Создан `public/llms.txt` (бренд, факты, цены, страницы, works.json, sitemaps). В корне mbezu.ru его отдать нельзя (Tilda), живёт на CDN.
