...
## Sprint 5 — статус
- **⛔ ОСТАНОВЛЕНО на Phase 1 (аудит, read-only), ДО любой правки прода. Tilda НЕ тронута.**
- **Stage A сделан** (чанки/CDN исправлены и проверены — годится для будущего).
- **🚨 Критический блокер: живой магазин — это НАТИВНЫЙ Tilda Store, а НЕ React-#root-приложение.** Sprint-5 предполагал обратное. Reconnect разрушил бы реальную коммерцию. Нужно продуктовое решение владельца.
