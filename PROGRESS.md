# Редизайн 2026 (по `../HANDOFF.md`)

## Статус
- **✅ Фазы 1–2 на live.** Деплой идёт через CDN: контейнеры Tilda ссылаются на
  стабильный `https://cdn.mbezu.ru/e/<page>.js`, поэтому коммит в `main` → GitHub
  Actions → CDN, и боевые страницы подхватывают новую сборку без правки Tilda.
- **⛔ Осталось: залить свежий prerendered-HTML в блоки Tilda** (`npm run push`).
  Сейчас в блоке лежит разметка от 04.09 — её видит робот и она же мелькает
  до монтирования React. Нужна живая сессия: `npm run tilda:login` (один вход
  руками, автологин намеренно не делается — он вызывал reCAPTCHA).

## Лог
- `[done] Правки Олега 20.09` — 2026-09-20
  - Контейнеры и мета залиты в Tilda (9 страниц, publish 200, снапшот
    `backup/tilda-snapshot/2026-09-20T16-45-13`) — в блоках теперь свежий
    prerender нового дизайна, а не разметка от 04.09.
  - Telegram: `t.me/mbezu_art` не существует (у аккаунта нет @username).
    Единая ссылка `ABOUT.contacts.telegramUrl` = `t.me/+79167641039` — в React,
    в блоке chrome-footer Tilda и в JSON-LD head-кода; 21 страница
    перепубликована, по всем адресам старых вхождений 0.
  - Заявки: у всех форм (A и B, обе страницы + корзина) в Tilda стоят
    интеграции email + telegram. Боевая проверка с главной — заявка
    `MB-260920-5ZUD` ушла, форма A подтвердила успех.
  - Нижняя навигация вернулась на мобиле; липкая панель закрывается крестиком
    (решение живёт в sessionStorage) и стоит над таб-баром.
  - Подборки «Подобрать / По сюжету» — крупные пилюли с обводкой.
  - Обрезанные карточки: сверка с `mbezu-cards-v4-webp.zip` нашла 7 файлов с
    отрезанным краем (ts-02 −68%, ts-06 −33%, mn-01 −25%, ts-05 −17%, mn-06,
    st-08, mn-03). Пересобраны из v4 вместе с @480/@960. Миниатюра на /tracking
    больше не режется (cover → contain).
  - Осталось владельцу: 10 работ, где размер в каталоге не сходится с
    пропорцией фото (MN-06, ST-02, ST-06, ST-07, ST-08, TS-01, TS-02, TS-04,
    TS-05, TS-06) — либо перепутана ориентация, либо привязано чужое фото.

- `[done] Фаза 1 — каркас, карточки, главная` — 2026-09-20
  - Одна шапка вместо двух навигаций: десктоп (полоса доверия + меню из 5 пунктов
    + телефон + Telegram + корзина + полоса прогресса), мобайл 61px с бургером.
    Переключение по `innerWidth < 900` в состоянии, не CSS-классом (§13.6).
  - `StickyBar` вместо `.tabbar` (дублировал плавающую корзину Tilda).
  - `ArtCard` — паспарту (§6): плашка размера на изображении, «Купить» отдельной
    ссылкой рядом с карточкой (§13.5), ховер только при `hover:hover`.
  - `reveal.ts` — scroll-reveal, счётчики, полоса прогресса на IntersectionObserver
    и слушателе scroll; `animation-timeline` не используется (§13.1–13.2).
  - Главная: форма в первом экране, плитка-разводка, текучие сетки, тизер серии
    без хака `.split(/(?<=.)s/)`, склонения через `plural()`, StatsRow не дублирует
    цифры hero, `scrollIntoView` убран, блок AR удалён вместе с чанком.
- `[done] Фаза 2 — каталог, бриф, карточка работы` — 2026-09-20
  - Каталог: сводка (работ / диапазон цен / размеров), пилюли серий со счётчиком,
    липкая полоса фильтров от реальной высоты шапки, плитка «напишем под ваш
    размер» последней ячейкой, SEO-текст в `<details>`.
  - Бриф: 5 шагов (Размер → Палитра → Бюджет → Сроки → Контакт), прогресс,
    возврат к пройденным шагам, липкое резюме на тёмном. Транспорт заявки прежний.
  - Карточка работы: паспарту 5/3, 4 ракурса, блок масштаба (стена 340×260,
    диван/дверь/комод), «Купить сейчас» → нативный Store, своя липкая панель.
  - `tilda-chrome.ts` прячет собственные меню и подвал Tilda на страницах витрины
    (корзина 706, каталог 776 и формы-приёмники не трогаются). Проверено на проде:
    видимых `<header>` 1, `<footer>` 1, горизонтальной прокрутки нет на 375 и 1440.
  - Пустой блок отзывов больше не рендерится (§1.13).
...
- `[done] Статичный JSON-LD на 7 посадочных` — раньше Breadcrumb/FAQPage/ItemList строились только runtime-скриптом (валидатор показывал лишь Person/Organization). Теперь блок `<script type="application/ld+json" id="mbezu-ld-<key>">` генерируется прогоном runtime-скрипта в headless Chromium (`scripts/_landings-static-ld.mjs --write`, podarok — `_landing-static-ld.mjs`), runtime не дублирует при наличии блока. /podarok в валидаторе: FAQPage(6), BreadcrumbList, ItemList с 17 Product. При правке текста FAQ/подборок на посадочной — перегенерировать блок тем же скриптом перед `tilda-landing-pages.mjs`. Все 7 посадочных проверены на живом: JSON парсится, guard стоит (у спальни/кабинета runtime — `add(d)`, guard добавлен отдельно); /kartina-v-gostinuyu в валидаторе: FAQPage(6), BreadcrumbList, ItemList с 6 Product и размерами (QuantitativeValue).
- `[checked] Обход сайта` (`scripts/_site-crawl.mjs`, YandexBot UA, паузы) — 43 URL из трёх sitemap: все 200, canonical = URL, og:image везде, ровно один H1 (у 4 статей журнала H1 только из JS — шаблон поста Tilda), дублей title нет; 44 внутренних ссылки вне sitemap — битых 0. Замечания: title >70 у /kartina-v-kabinet (80), /catalog/botanika (73), /kartina-v-gostinuyu (72), /kartina-v-spalnyu (70); description >160 у /kartina-v-gostinuyu (169), /podarok (166) — укорочены в `seo/landings.json`.
- `[checked] validator.schema.org` (под Google, без входа) — /podarok: Organization, BreadcrumbList, ItemList, Person, FAQPage — 0 ошибок, 0 предупреждений. Google Rich Results Test требует вход в Google — недоступен. Вебмастер: «Проверка мобильных» и «Уведомления» — «сервис временно недоступен» (Яндекс), пропущено.
- `[checked] Длинные задачи каталога` — при CPU×4 сумма 1,25 с, максимум 210 мс, DCL 1,0 с, load 1,7 с; «2,2 с unattributable» в Lighthouse — артефакт симуляции. content-visibility не внедрял: реальные цифры в норме.
- `[checked] Техника` — несуществующие URL (страница, товар, пост) отдают честные 404 (не soft-404); http→https и www→без www — 301; /catalog/, /Catalog, ?utm — 200 с canonical на чистый URL (склейка работает); /index.html, /home — 404; RSS у Tilda нет (/rss, /feed — 404). Alt: у всех наших картинок есть осмысленный alt, единственный пустой — плашка «Made on Tilda» (снимается тарифом). Уведомления Вебмастера и «Проверка мобильных» — по-прежнему «сервис временно недоступен».
- `[idea→Олег] Спецблоки Яндекса` — Вебмастер «Услуги и предложения в поиске» (`/admin-feeds-main/admin-feeds/`) принимает YML-фиды только категорий Исполнители/Врачи/Образование/Вакансии/Недвижимость/Транспорт. Для «картина на заказ» возможен фид «Исполнители» (публичный профиль специалиста — решение владельца). Товарные предложения с ценами — только через Яндекс Маркет (регистрация продавца).
- `[checked] Воронка Метрики end-to-end` (`scripts/_goal-probe4.mjs`) — клик «Корзина» в шапке витрины → `ym reachGoal cart_open` → ping `watch/111308276?page-url=goal://mbezu.ru/cart_open` ушёл; нативная корзина открылась. На странице товара Tilda пушит `dataLayer` ecommerce `detail` и `add` (клик «Купить»). В Метрике все 4 цели на месте (ID 608170842/608177562/608178721/608179786). Ловушка: Playwright-клик по `a[href="/cart"]` попадал в скрытую ссылку (их 4: шапка, таб-бар, подвал) — кликать видимую.
- `[fixed] llms.txt` — в отчёте и PROGRESS (строка «Аудит r2») значился как сделанный, но файла в репо не было (только слепки аудита) → cdn.mbezu.ru/llms.txt отдавал 404. Создан `public/llms.txt` (бренд, факты, цены, страницы, works.json, sitemaps). В корне mbezu.ru его отдать нельзя (Tilda), живёт на CDN. Выложен: https://cdn.mbezu.ru/llms.txt — 200 (deploy:cdn падает с libuv-assert на выходе, но публикация проходит; повторный прогон подтвердил).
- `[checked] Статьи журнала` (`scripts/_post-links-dom.mjs`) — 4 поста: 258–479 слов, 4–5 подзаголовков, 2–5 внутренних ссылок (каталог/серии/бриф/подарок/другая статья), внешних 0; H1 и title совпадают. Слабое место: внутри текста нет ни одной картинки (только обложка) — идея: html-блок с картой работы и ссылкой после первого раздела, для очереди — добавить в JSON статей.
- `[done] Картинки работ внутри статей журнала` — `scripts/_journal-inline-images.mjs`: html-блок `<figure class="mbezu-inline-fig">` (карта @960, ссылка на товар, подпись «название, холст, масло, размер — серия») после первого абзаца. Живые 4 поста (гостиная→Волна. Сепия, постер→Тропические листья, сертификат→Обидуш, монохром→Камни на берегу) — проверено в DOM: картинка грузится, ширина 720, ссылка верная, посты активны; очередь 6 статей — блок вставлен в JSON по cover. Ловушки posts_Edit (сброс active, флаги «n»→включено, HTML-экранирование) — в памяти tilda-feeds-contract.
- `[done] 152-ФЗ: уведомление о cookie` — на сайте не было плашки (политика cookie/Метрику описывает). В head Tilda добавлен `CK_SNIPPET` (`scripts/tilda-head-code.mjs`, маркер «MBezu · cookie-notice»): фиксированная плашка снизу «Сайт использует cookie и Яндекс Метрику… соглашаетесь с политикой обработки персональных данных» + «Понятно», согласие в `localStorage.mbezu_cookie_ok`, без regex/слэшей. Republish 21 стр. Проверено (`scripts/_cookie-verify.mjs`): показ на главной (desktop) и товаре (mobile), ссылка /legal?section=privacy, клик убирает и запоминает, после перезагрузки не показывается, CLS от плашки 0, корзину не перекрывает. Скриншоты `audit/cookie-desktop.png`, `audit/cookie-mobile.png`.
- `[fixed] Шапка товара на мобильном` — панель Tilda `.t-popup__close` (стрелка «назад», 50 px) лежала поверх нашей JS-шапки: логотип и «Корзина» обрезаны. CSS в head (`html[data-mbezu=prod]` ≤760px): `#mbezu-prod-header{padding-top:50px}`, резерв `#allrecords{padding-top:139px}`. Republish v7. Проверено: шапка 139 px, стрелка сверху, логотип/корзина целиком; CLS товара 0,137 → 0,038.
- `[done] verify-live → 210 проверок` (агент, параллельный режим 03.09) — +105: на 11 страницах head-маркеры perf-preload (≥4 preload шрифтов), preconnect cdn, perf-metrika/mbzGo, cookie-notice/#mbezu-ck, целый inline-скрипт data-mbezu (нет `replace(//+$/`), `:has(input[name="lead_ref"])`; title ≤70 на 16 страницах; на 7 посадочных status 200 + JSON-LD `#mbezu-ld-*` с FAQPage+BreadcrumbList + description ≤160; llms.txt 200. Паузы 400 мс перед запросами. На границе: /catalog/gory title = 70, /catalog/more и /kartina-v-kabinet description = 160.
- `[done] Очередь журнала: +3 статьи` (агент) — «Как повесить картину: высота, крепёж, свет» (внутри «Свобода» 70×50, обложка ts-04), «Картина в кабинет руководителя и офис» (внутри «Шторм» 30×30, обложка mn-01), «Картины маслом с горами и городами» (внутри «Обидуш», обложка st-01). 490–510 слов, 5 H2, 4 внутренние ссылки, факты только из брифа/works.json, без слэшей (атрибуты в одинарных кавычках). Очередь: 10 записей, опубликована 1 → 9 впереди, ритм 1 статья в 2–3 недели (`npm run journal:next`).
...
- `[done] a11y: клавиатура и семантика` (агент, `scripts/_a11y-keyboard.mjs`, live 5 страниц → правки в src/) — логотип шапки был `<div onClick>` (не в Tab, без имени) → `<a href="/" aria-label>`; `nav` шапки/крошек/документов получили aria-label; поля форм заявки/подписки/отзыва/брифа — aria-label (раньше только placeholder); у поля подписки inline `outline:none` убивал единственный фокус-индикатор → кольцо в `styles.css`; переключатели размера/стиля/палитры/сроков и фильтры каталога — `aria-pressed`, табы /legal — `aria-current`; ColorPicker: два `role=slider` без tabindex → `tabIndex=0` + стрелки; бегущая строка и декоративный «MB» — `aria-hidden`; звёзды рейтинга — `role=img`. Локально после правок: полей без label 0, «кликабельно, но не фокусируемо» 0, фокус без индикатора 0. Осталось на стороне Tilda: cookie-плашка последняя в Tab (смягчено `scroll-padding-bottom`), корзина t706 без role=dialog и переноса фокуса.
- `[done] Мокапы в интерьере на 3 посадочных` (агент, `scratchpad/compose.py`; фоны Adobe Stock free 545457016/321935787/340844292) — масштаб по мебели: гостиная 15,6 px/см (диван 200 см) → MN-01 «Волна. Сепия» 100×60 из `works/mn-01.jpg` с выправленной перспективой; спальня 11,6 px/см (изголовье 180 см) → TS-03 «Кувшинки» 50×50 (честно мелковата — вариант с TS-02 50×90 делается); кабинет 25 px/см (iMac/лампа/часы, стол ≈155 см) → ST-07 «Крыши старого города» панорама над монитором. Файлы `public/assets/interior/{gostinaya,spalnya,kabinet}.webp` (+@960/@480, 73–246 KB), превью `audit/interior-mockups/*.jpg`; `<figure>` с srcset/alt/подписью и ссылкой на товар — после первого экрана в `content/kartina-v-*.html`.
- `[fixed] alt ST-07 на двух посадочных` — «вертикальная картина/пейзаж» при горизонтальной панораме (карточка 2,9:1, Tilda: «Панорамная») → «горизонтальная».
- `[question→Олег] Размеры и ориентация работ расходятся` (`scripts/_orient-live.json`, с живых страниц товара) — в Tilda у 16 работ размер идёт как ширина×высота (MN-01 100×60 «Горизонтальная», MN-05 30×40 «Вертикальная»…), но: **ST-07** 25×70 «Панорамная» и **TS-02** 50×90 «Горизонтальная» — записаны высота×ширина; **TS-01** 70×50 «Горизонтальная», **TS-04** 40×30 «Горизонтальная», **TS-05** 18×24 «Вертикальная» — карточка/фото противоположной ориентации. ST-05 и тондо без поля «Ориентация». Нужно подтверждение реальных размеров пяти работ; на сайте пока как в Tilda.
- `[done] Lighthouse a11y 96 → наши провалы контраста закрыты` — подписи серий на карточках (`atoms.tsx`, opacity .8 → .86: 4,37/4,48 → ≥5,0 на #ede5d6), водяной знак «M.B» главной через `::before{content:attr(data-wm)}` (axe не считает псевдоэлемент текстом). Остался только бейдж «Made on Tilda» (#666 на #000, 3,65).
- `[found] Бейдж Tilda защищён от CSS` — правило `#tildacopy .t-tildalabel__…{color:#a3a3a3}` в head-коде Tilda сохранила с кириллическими двойниками букв в селекторе (`#tildacоpy .t-tildalаbеl__txtleft`) — мёртвый CSS. Откатил; 96 в a11y остаётся до годового тарифа (память tilda-block-contracts).
- `[done] Спальня v2` — «Зеркало леса» 50×90 (из `works/ts-02.jpg`, кроп по граням холста, 1044×580 px при 11,6 px/см) вместо «Кувшинок» 50×50: занимает половину изголовья, читается как картина над кроватью. Живая /kartina-v-spalnyu: alt/подпись/ссылка на товар обновлены, CDN отдаёт новые файлы.
- `[fixed] ColorPicker: role=slider без aria-valuenow` (воркфлоу-ревью правок src/, линза a11y; две другие линзы и опровергатели упали в лимит сессии) — у поля «Насыщенность и яркость» были только aria-valuetext=hex; добавлены aria-valuenow (насыщенность, %), valuemin/valuemax и valuetext «насыщенность N%, яркость M%, цвет #hex». Правило axe aria-required-attr закрыто.
- `[done] Перелинковка, часть 1: «Смотрите также» на 7 посадочных и /journal` (04.09; граф ссылок по слепку 43 URL — воркфлоу, `scripts/_linkgraph.mjs` → `scratchpad/crawl/linkgraph.json`) — до правок из контента (без шапки/подвала) на /journal вели 2 ссылки, на /about 3, на статьи журнала 1–2, на подборки море/ботаника/горы по 4, на главную 4. В каждый из 8 файлов `content/*.html` добавлен один блок из 4 ссылок (заголовок h2 «Смотрите также», список с меткой cat-no: Подобрать/Подборка/Серия/Статья), цели — страницы с наименьшим числом входящих, без дублей уже имеющихся в тексте ссылок и без ссылки на себя; все href из sitemap, ld+json и скрипты не тронуты. Залито в Tilda, на живых страницах проверено (блок есть на всех 8).
- `[checked] og:image по всем 43 URL` (`scratchpad/crawl/og-check.json`) — все 24 уникальные картинки отдают 200, абсолютные https, ширина ≥843 px; 10 карточек товаров вертикальные/квадратные (это сама работа — норма). Замечаний по мета: у 4 статей журнала и /journal одинаковый description (шаблон Tilda для ленты) и заголовок статьи в div вместо h1 — шаблон Tilda, не правится.
- `[found] Карточки товаров — 36 подтверждённых находок из 262` (7 агентов-аналитиков + 6 опровергателей, только medium/high): (1) крошки, ссылка на серию и «Ещё из серии» только через JS из works.json, в статике из карточки ведут лишь две ссылки на /catalog — BreadcrumbList JSON-LD добавлен в head-код (runtime, см. ниже); (2) описания: ~45 % текста — шаблонные фразы, общие для всех 22 товаров, у «Некуда спешить», «Шторм», «Тропические листья», «Кувшинки» уникальны 2 абзаца — нужны авторские тексты; (3) характеристика «Техника: Холст, масло» у всех 22, хотя у MN-02, MN-03, MN-04, ST-03, ST-04 в тексте «холст на картоне» — поправить характеристику в Store; (4) «Обидуш» без артикула (itemprop sku); (5) корзина: заголовок «Payment method» по-английски — исправлено в head-коде; (6) галерея товара — одно изображение (карточка сетки) фоном div без alt — нужны фото от Милы; (7) **«Вершина» (MN-02)** — в data.ts `hidden:true` («своего фото нет»), но в Tilda Store открыта к продаже (25 000 ₽, InStock) и в sitemap-store при 0 входящих ссылок: витрина её не показывает, крошки/«Ещё из серии» не строятся. Решение владельца: дать фото и открыть на витрине либо скрыть товар в магазине.
- `[question→Олег] + «Раковина» (MN-06)` — по подтверждённой находке: характеристика «Ориентация: Вертикальная» при 24×30 и горизонтальной картинке; добавлена к списку из пяти работ (память works-dimensions-conflicts).
- `[done] Перелинковка, часть 2: витрина` (04.09; по предложениям агента, применено мной) — новый атом `LinkStrip` (`atoms.tsx`: метка cat-no + ссылки через «·»). Главная: под сеткой серий строки «Подобрать: в гостиную · в спальню · в кабинет · в подарок» и «По сюжету: море и волны · цветы и растения · горы», в блоке упаковки ссылка на /podarok. Каталог (только общий, не серии): перед CTA секция «Подобрать / По сюжету / Журнал». /about: абзац «Журнал» в карточке мастерской и подводка к /podarok после «ритуала получения». /commission: под условиями — «нужно к дате — готовые работы: в подарок / каталог», после FAQ — статья и подборки по комнатам. Пункт меню «В подарок» не добавлял (дизайн-решение владельца). Деплой 210/210, на живом домене все строки есть.
- `[done] BreadcrumbList JSON-LD на страницах товара` — в head-коде `productSeries()` после вставки крошек добавляет `<script type=application/ld+json id=mbezu-ld-crumbs>` (MBezu → Каталог → Серия → Работа, из works.json). Runtime, как и сами крошки; Яндекс исполняет JS.
- `[dropped] Снипет «Payment method → Способ оплаты»` — находка аудита была по статическому HTML: в живом DOM корзины Tilda заголовок уже «Способ оплаты» (проверено Playwright). Снипет из head-кода убран, `cartRu()` оставлен как чистильщик.
