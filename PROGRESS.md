...
- **Осталось (живой нативный Store / глобальная настройка / решение):** (a) импорт цен+фото в нативный Store (`02-tilda-store-import-v3.csv` готов), (b) золото на нативных страницах = правка глобального `/custom.css` (готов `backup/custom-gold.css`), (c) цена ST-08 (держу 75 000 — жду слова), (d) 3C чекаут — отдельно.
...
- `[ready, ждёт go] Нативный Store + глобальный CSS` — 2026-06-24
  - `02-tilda-store-import-v3.csv` (22 строки): цены=bizar, Photo=CDN webp; добавлены ST-01/ST-02/TS-01 (их не было в Store); MN-03 фото оставлено старым (карточки нет); цены MN-05/ST-03/ST-08 не тронуты.
  - `backup/custom-gold.css`: глобальный `/custom.css` с золотом (6 замен терракоты → 0 осталось) для нативных страниц.
  - **Пауза перед заливкой в живой Store (деньги) и правкой глобального CSS (blast-radius = весь сайт, вкл. checkout): нужен способ (я через Playwright vs Олег) + решение по ST-08.**
