...
- **Осталось:** (a) золото на нативных страницах = правка глобального `/custom.css` (готов `backup/custom-gold.css`; механизм правки настроек Tilda не автоматизирован), (b) 3C чекаут — отдельно, (c) data.ts ST-08=15000 уедет со след. деплоем.
...
- `[ready, ждёт способа] Глобальный CSS — золото на нативных` — 2026-06-24
  - `backup/custom-gold.css`: `/custom.css` с золотом (6 замен → 0 терракоты). Применить = правка site-wide CSS в настройках Tilda (механизм ещё не автоматизирован; blast-radius = весь сайт вкл. checkout).
